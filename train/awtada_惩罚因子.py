...
def adaBoostTrainDS(dataArr, classLabels, k, numIt = 80):	#k为规模系数
	"""
	使用AdaBoost算法提升弱分类器性
	Parameters:
		dataArr - 数据矩阵
	...
	for it in range(numIt):	
		...
		csl=np.log(error_times)/np.log(20)
...
if __name__ == '__main__':
	...
	dataArr, LabelArr = loadDataSet('威train.csv')
	time_start = time.time() #开始计时
	weakClassArr, aggClassEst,wste_time = adaBoostTrainDS(dataArr, LabelArr,5)
	time_end = time.time()    #结束计时
	testArr, testLabelArr = loadDataSet('威test.csv')
