...
def testRbf(train_data,test_data,k1):
    """
    测试函数
    Parameters:
        k1 - 使用高斯核函数的时候表示到达率
    ...
    dataArr,labelArr = loadDataSet(train_data)                        #加载训练集
    print(np.mat(dataArr).shape,np.mat(labelArr).shape)
    b,alphas = smoP(dataArr, labelArr, 200, 0.0001, 20, ('rbf', k1))       #根据训练集计算b和alphas
    datMat = np.mat(dataArr); labelMat = np.mat(labelArr).transpose()
    svInd = np.nonzero(alphas.A > 0)[0]                                     #获得支持向量
    time_end = time.time()    #结束计时
    sVs = datMat[svInd]                                                     #将支持向量的样本值转变为矩阵形式
    labelSV = labelMat[svInd];                                              #将支持向量的标签值转变为矩阵形式
    # print("支持向量个数:%d" % np.shape(sVs)[0])
    m,n = np.shape(datMat)
    errorCount = 0
    for i in range(m):  #这里可以用矩阵进行优化，就不用循环了
    ...
    dataArr,labelArr = loadDataSet(test_data)                       #加载测试集
    errorCount = 0
    datMat = np.mat(dataArr); labelMat = np.mat(labelArr).transpose()       
    m,n = np.shape(datMat)
    TP=FN=FP=TN=0
    predict_mat=np.mat(np.zeros((m,1)))
    for i in range(m):
        kernelEval = kernelTrans(sVs,datMat[i,:],('rbf', k1))               #计算各个点的核            
        predict=kernelEval.T * np.multiply(labelSV,alphas[svInd]) + b       #根据支持向量的点，计算出超平面，返回预测结果
        predict_mat[i,0]=predict
        if np.sign(predict) != np.sign(labelArr[i]): 
            errorCount += 1        #返回数组中各元素的正负符号，用1和-1表示，并统计错误个数
        if np.sign(predict) > 0 and  np.sign(labelArr[i]) > 0: 
            TP+=1
        elif np.sign(predict) < 0 and  np.sign(labelArr[i]) > 0: 
            FN+=1
        elif np.sign(predict) > 0 and  np.sign(labelArr[i]) < 0: 
            FP+=1
        elif np.sign(predict) < 0 and  np.sign(labelArr[i]) < 0: 
            TN+=1
    # print(predict_mat)
    plotROC(predict_mat.T, labelArr)
    print("测试集错误率: %.2f%%" % ((float(errorCount)/m)*100))           #打印错误率
    print("%d %d %d %d" % (TP,FN,FP,TN))
    print("精确率为: ", TP/(TP+FP))
    print("召回率为: ", TP/(TP+FN))
...
if __name__ == '__main__':
    testRbf('威train.csv','威test.csv',300)

#测试集错误率为0，测试集错误率为50%左右,很可能是过拟合了，全部都判断为负
#如果过拟合了可以尝试吧gamma设大一点，比如100,500